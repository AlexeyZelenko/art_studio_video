...
import { VideoService, VideoServiceFormData } from '../../types/video';

interface VideoServiceFormProps {
  service?: VideoService | null;
  onClose: () => void;
  onSuccess: () => void;
}

const VideoServiceForm: React.FC<VideoServiceFormProps> = ({ service, onClose, onSuccess }) => {
  const { addVideoService, updateVideoService } = useVideo();
  const [formData, setFormData] = useState<VideoServiceFormData>({
    title: service?.title || '',
    description: service?.description || '',
    price: service?.price || '',
    features: service?.features || [''],
    iconName: service?.iconName || 'Video',
  ...
  const iconOptions = [
    'Video', 'Camera', 'Heart', 'Briefcase', 'Users', 'Zap', 
    'Star', 'Award', 'Crown', 'Sparkles', 'Play', 'Film'
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (service) {
        await updateVideoService(service.id, serviceData);
      } else {
        await addVideoService(serviceData);
      }
      
      onSuccess();
    } catch (error: any) {
      setError('Помилка при збереженні послуги');
      console.error('Error saving video service:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="flex justify-between items-center mb-4 md:mb-6">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">
            {service ? 'Редагувати відео послугу' : 'Додати нову відео послугу'}
          </h2>
          <button
            onClick={onClose}
